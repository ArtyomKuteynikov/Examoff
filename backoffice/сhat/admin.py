...
class EmployeeAdmin(admin.ModelAdmin):
    ...
    exclude = ['first_name', 'last_name', 'last_login', 'username', 'is_superuser', 'is_staff', 'groups', 'user_permissions', 'date_joined', 'uuid', 'show', 'password']
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).filter(is_staff=False, show=True)

    def save_model(self, request, obj, form, change):
        if not change:
            obj.set_password_hash()
        obj.is_staff = False
        super().save_model(request, obj, form, change)
